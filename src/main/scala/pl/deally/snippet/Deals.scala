...
class Deals {
  ...
  def edit(in: NodeSeq): NodeSeq = {
    ...
    Deal.find(By(Deal.id, dealId)) match {
      case Full(deal) => {
        ...
        var selectedCat = 0L
        val cats = 
          List(("0","-")) ::: Category.findAll.map(cat =>(cat.id.toString, cat.title.is)).toList
      ...
      deals.flatMap(
        deal => {
          ...
          bind("deal", in,
           "title" -> deal.title, 
           "titleLink" -> <a href={"/deal/"+deal.id+"/"+Deal.toPermalink(deal.title.toString)+".html"}>{deal.title}</a>,
           "text" -> HtmlHelpers.trimAfter(deal.text, 140, "..."),
           "fulltext" -> deal.text,
           "expire" -> deal.expire,
           "actions" ->  {
                         <span class="actions">
  ...
  def outLink(in: NodeSeq): NodeSeq = {
    val dealId: Long = S.param("dealid").map(_.toLong) openOr S.redirectTo("/error")
    var deal: Deal = Deal.create
    Deal.find(By(Deal.id, dealId)) match {
      case Full(d) => deal = d
      case _ => S.redirectTo("/error")
    }
    if(deal.url.startsWith("http")) {
      S.redirectTo(deal.url)
    } else {
      in
    }
  }
